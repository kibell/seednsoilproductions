import { linkPageData, socialLinks } from "../data/content";
import "./Links.css";
      ...
      <div className="links-container">
        ...
        <div className="links-brand">
          <div className="links-avatar">
            <span className="links-avatar-icon">✦</span>
          </div>
          <h1 className="links-title">{linkPageData.title}</h1>
          <p className="links-subtitle">{linkPageData.subtitle}</p>
        </div>

        {/* Link buttons */}
        <div className="links-list">
          {linkPageData.links.map((link) => (
            <a
              key={link.id}
              href={link.href}
              className={`link-btn link-btn-${link.style}${link.disabled ? " link-btn-disabled" : ""}`}
              style={
                link.platform && platformColors[link.platform]
                  ? {
                      background: platformColors[link.platform],
                      color: ["spotify", "amazon"].includes(link.platform) ? "#000" : "#fff",
                    }
                  : {}
              }
        ...
        <div className="links-socials">
          ...
          <a href={socialLinks.spotify} aria-label="Spotify">
